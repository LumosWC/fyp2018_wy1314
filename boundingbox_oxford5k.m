...
load rmac_oxford5k.mat
load conv3d_oxford5k.mat
...
imfn = strcat(dataset_folder,gnd_oxford5k.imlist{im_index} ,'.jpg');
image = imread (imfn);
imshow(image);
...
object = crop_qim (imfn,gnd_oxford5k.gnd(query).bbx);
figure;
...
[distance,ranks] = sort(rmac_oxford5k'*qvec, 'descend');
...
conv3d_toplist = arrayfun(@(y)floor((15+((conv3d_oxford5k{y}>=128)*128+conv3d_oxford5k{y}.*(conv3d_oxford5k{y}<128)))/16), ids_toplist, 'un', 0);
...
figure
for i = 1:9 % the most similar reranked results
    imfn = strcat (dataset_folder,char (gnd_oxford5k.imlist(ranks_rerank(i))),'.jpg');
