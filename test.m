...
if ~use_rmac % when using mac
	if rerank % additionally keep the 3D tensor of responses in case re-ranking is performed
		...
		[vecs] = cellfun(@(x) vecpostproc(x), vecs, 'un', 0); % L2 normalisation
	else
...
else % when using rmac - choose this one
	if rerank % additionally keep the 3D tensor of responses in case re-ranking is performed
...
vecs = cellfun(@(x) vecpostproc(apply_whiten (x, Xm, eigvec, eigval)), vecs, 'un', 0);% Step 3
if use_rmac
