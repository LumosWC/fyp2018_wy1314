...
gnd_test = load(['local_data//', '/gnd_', 'paris6k', '.mat']);
gnd_test = gnd_test.gnd_paris6k;
gnd_train = load(['local_data//', '/gnd_', 'oxford5k', '.mat']);
gnd_train = gnd_train.gnd_oxford5k;
...
[vecs, conv3d] = cellfun(@(x) rmac_regionvec(imread(x), net, L), im_fn_test, 'un', 0); % Step 1
...
qimlist = {gnd_test.imlist{gnd_test.qidx}}; % query image list
...
map = compute_map (ranks, gnd_test.gnd); % can be used in my own code
...
fprintf('This experiment is trained with Oxford5k and evaluate on Paris6k \n');